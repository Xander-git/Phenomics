...
class ColonyProfile:
    def __init__(self, img, sample_name,
        ...
        self.results = None
        self.cp_connection = CellProfilerApi()
        if auto_run:
            self.run_analysis()
    ...
    def run_analysis(self):
        results = self.cp_connection.run(self.input_img, f"{self.sample_name}")
    ...
    def plotAx_segmentation(self, ax, cmap="viridis"):
        with plt.ioff():
            if self.status_validity:
                ...
                ax.imshow(
                    self.cp_connection.obj_set.get_objects(f"{self.colony_name}").segmented,
                    cmap=cmap
                )
                ax.set_title("Segmented Colony")
            else:
    ...
    def plotAx_unfiltered(self, ax,cmap="viridis"):
        with plt.ioff():
            if self.status_validity:
                ...
                ax.imshow(
                    self.cp_connection.obj_set.get_objects(f"{self.filled_name}").segmented,
                    cmap=cmap
                )
                ax.set_title("Segmented Colony")
            else:
    ...
    def _pull_object_info(self):
        self.segmentation = self.cp_connection.obj_set.get_objects(self.colony_name).segmented
        self.unfiltered_segmentation = self.cp_connection.obj_set.get_objects(self.filled_name).segmented
